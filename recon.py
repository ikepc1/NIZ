...
def base_fit_params() -> pd.DataFrame:
    ...
    l.append({'name':'Nmax', 'initial_value': 1.e7, 'limits':(1.e5,1.e9), 'fixed': False})
...
#     def get_input_indices(self) -> np.ndarray:
...
#         paes = np.zeros(len(self.active_counters))
#         for nfit in nfits:
...
#         ts = np.zeros(len(self.active_counters))
#         tes = np.zeros(len(self.active_counters))
